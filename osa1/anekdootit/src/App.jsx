import { useState } from 'react'

const Prompt2 = (props) => {
  return (
    <div>
      <p>{props.anecdote}</p>
      <p>has {props.points} votes</p>
    </div>
  )
}

const Prompt = (props) => {
  return (
    <div>
      <p>{props.anecdote}</p>
      <p>has {props.points} votes</p>
    </div>
  )
}

const Button = (props) => (
  <button onClick={props.handleClick}>{props.text}</button>
)

const App = () => {
  const anecdotes = [
    'If it hurts, do it more often.',
    'Adding manpower to a late software project makes it later!',
    'The first 90 percent of the code accounts for the first 90 percent of the development time...The remaining 10 percent of the code accounts for the other 90 percent of the development time.',
    'Any fool can write code that a computer can understand. Good programmers write code that humans can understand.',
    'Premature optimization is the root of all evil.',
    'Debugging is twice as hard as writing the code in the first place. Therefore, if you write the code as cleverly as possible, you are, by definition, not smart enough to debug it.',
    'Programming without an extremely heavy use of console.log is the same as if a doctor would refuse to use x-rays or blood tests when diagnosing patients.',
    'The only way to go fast is to go well.'
  ]
   
  const [selected, setSelected] = useState(0)
  const [points, setPoints] = useState(new Array(anecdotes.length).fill(0))
  const [maxpoints, setMax] = useState(0)

  const handleClick = () => {
    const item = Math.floor(Math.random() * anecdotes.length)
    setSelected(item)
  }

  const handleVote = () => {
    const copy = [...points]
    copy[selected] += 1
    setPoints(copy)
    if (copy[selected] > Math.max(...points)) {
      setMax(selected)
    }
  }

  return (
    <div>
      <h1><b>Anecdote of the day</b></h1>
      <Prompt anecdote={anecdotes[selected]} points={points[selected]}/>
      <Button handleClick={() => handleVote()} text="vote" />
      <Button handleClick={() => handleClick()} text="next anecdote" />
      <h1><b>Anecdote with most votes</b></h1>
      <Prompt2 anecdote={anecdotes[maxpoints]} points={points[maxpoints]}/>
    </div>
  )
}

export default App
